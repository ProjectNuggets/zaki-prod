...
- `ZAKI_APP_CHAT_DAILY_PROMPT_LIMIT` (optional free-user daily cap for normal app chat, default `10`)
- `ZAKI_APP_CHAT_DAILY_PROMPT_BUCKET` (optional bucket name for normal app chat quota, default `app_chat`)
- `ZAKI_ANONYMOUS_SPACES_DAILY_PROMPT_LIMIT` (anonymous Spaces daily cap, default `10`)
- `ZAKI_ANONYMOUS_SPACES_DAILY_PROMPT_BUCKET` (anonymous Spaces quota bucket, default `anonymous_spaces`)
- `ZAKI_ANONYMOUS_SPACES_MODEL` (Together model used for anonymous Spaces chat, default `openai/gpt-oss-120b`)
- `ZAKI_BOT_WEEKLY_PROMPT_LIMIT` (optional free Agent preview weekly cap, default `10`; falls back to the old daily env name for compatibility)
- `ZAKI_BOT_WEEKLY_PROMPT_BUCKET` (optional Agent preview quota bucket, default `zaki_bot_weekly`)
- `ZAKI_LEARNING_WEEKLY_PROMPT_LIMIT` (optional free Learn preview weekly cap, default `10`; falls back to the old daily env name for compatibility)
- `ZAKI_LEARNING_WEEKLY_PROMPT_BUCKET` (optional Learn preview quota bucket, default `learning_weekly`)
...
- `GOOGLE_OAUTH_REDIRECT_URI` (registered callback URL, defaults to `${ZAKI_PUBLIC_URL}/api/auth/google/callback`)
- `GOOGLE_OAUTH_STATE_SECRET` (HMAC secret for OAuth state; production must set an explicit secret)
- `ZAKI_BILLING_PROVIDER` (`stripe`, `creem`, `paddle`, `external`, or `none`; default `stripe`)
- `STRIPE_PRICE_AGENT_MONTHLY` (ZAKI Agent monthly Stripe price)
- `STRIPE_PRICE_LEARN_MONTHLY` (ZAKI Learn monthly Stripe price)
- `STRIPE_PRICE_COMPLETE_MONTHLY` (ZAKI Complete monthly Stripe price)
- `STRIPE_BILLING_PORTAL_CONFIGURATION` (Stripe Customer Portal configuration created by `npm run billing:ensure-commercial-catalog`; required for Agent/Learn to Complete upgrades)
...
- `GET /api/usage/quota?surface=app_chat|zaki_bot|learning` — authenticated quota status (Spaces daily, Agent/Learn weekly; defaults to `app_chat`)
