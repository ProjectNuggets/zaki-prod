...
function detailKind(item: MinutesItem): MinutesMetadata["kind"] {
  return "format" in item.content && item.content.format === "summary" ? "summary" : item.kind;
}
...
export function MinutesPage({ controlsEnabled = false }: { controlsEnabled?: boolean }) {
  ...
  const sessionQueryScope = useRef({ token: sessionToken, revision: 0 });
  if (sessionQueryScope.current.token !== sessionToken) {
    sessionQueryScope.current = {
      token: sessionToken,
      revision: sessionQueryScope.current.revision + 1,
    };
  }
  const sessionRevision = sessionQueryScope.current.revision;
  ...
  const index = useQuery({
    queryKey: ["minutes", "index", sessionRevision],
    queryFn: () => listMinutes({ limit: 50 }),
    retry: false,
    gcTime: 0,
    // ponytail: refetchInterval pauses itself on hidden tabs (refetchIntervalInBackground defaults false),
    // so "while visible" needs no listener; staleTime:0 stops the app-wide 5min default swallowing focus refetches.
    staleTime: 0,
    refetchInterval: 30_000,
    refetchOnWindowFocus: true,
  });
  const detail = useMutation({
    mutationFn: ({ itemId, variant }: { itemId: string; variant: "full" | "summary"; sessionRevision: number }) => readMinutesItem(itemId, variant),
    onMutate: ({ itemId }) => setSelected(itemId),
  });
  const search = useMutation({
    mutationFn: ({ query, cursor }: { query: string; cursor?: string; sessionRevision: number }) => cursor
    ...
    onSuccess: (page, request) => setSearchSession((current) => {
      if (request.sessionRevision !== sessionQueryScope.current.revision) return current;
  ...
  const loadOlder = useMutation({
    mutationFn: ({ cursor }: { cursor: string; sessionRevision: number }) => listMinutes({ cursor, limit: 50 }),
    onSuccess: (page, request) => setPagination((current) => request.sessionRevision === sessionQueryScope.current.revision ? ({
  ...
  const openDetail = (itemId: string, variant: "full" | "summary" = "full") => detail.mutate({ itemId, variant, sessionRevision });
  const currentDetail = detail.variables?.sessionRevision === sessionRevision;
  const currentSearch = search.variables?.sessionRevision === sessionRevision;
  const currentLoadOlder = loadOlder.variables?.sessionRevision === sessionRevision;
  const indexAuthError = isMinutesAuthError(index.error);
  const authError = indexAuthError
    || (currentSearch && isMinutesAuthError(search.error))
    || (currentDetail && isMinutesAuthError(detail.error))
    || (currentLoadOlder && isMinutesAuthError(loadOlder.error));
  const items = useMemo(() => {
  ...
  const selectedMetadata = selected
    ...
    : undefined;
  const detailTooLarge = currentDetail && detail.error instanceof MinutesApiError && detail.error.code === "minutes_item_too_large";
  ...
  return <main className="min-h-full bg-[var(--v2-bg)] p-4 text-[var(--v2-ink-1)] md:p-6" data-product-id="minutes">
    <div className="mx-auto max-w-7xl space-y-5">
      ...
      {!authError ? <section className="border border-[var(--v2-hairline)] bg-[var(--v2-bg-raised)] p-3">
        <form
          ...
          onSubmit={(event) => {
            ...
            if (query) search.mutate({ query, sessionRevision });
          }}
        >
          ...
          <V2Button type="submit" size="sm" disabled={!searchQuery.trim() || (currentSearch && search.isPending)}>
            {currentSearch && search.isPending ? t("minutes.searching", { defaultValue: "Searching…" }) : t("minutes.searchAction", { defaultValue: "Search" })}
          </V2Button>
        </form>
        <div aria-live="polite">
        {currentSearch && search.isError ? <p className="mt-3 text-xs text-[var(--v2-danger)]">{t("minutes.searchError", { defaultValue: "Search is unavailable. Your meeting list is still here." })}</p> : null}
        {searchSession ? <div className="mt-3 border-t border-[var(--v2-hairline)] pt-3">
          ...
          {searchSession.truncated && searchSession.next_cursor ? <V2Button className="mt-3" size="sm" disabled={currentSearch && search.isPending} onClick={() => search.mutate({ query: searchSession.query, cursor: searchSession.next_cursor, sessionRevision })}>{currentSearch && search.isPending ? t("minutes.loadingMoreSearch", { defaultValue: "Loading more results…" }) : t("minutes.loadMoreSearch", { defaultValue: "Load more search results" })}</V2Button> : null}
        </div> : null}
        </div>
      </section> : null}
      ...
      {index.isError && !indexAuthError && !index.data ? <V2Panel><V2PanelHead title={t("minutes.errorTitle", { defaultValue: "Minutes could not be loaded" })} meta={t("minutes.readFailedMeta", { defaultValue: "Read failed" })} /><V2PanelBody className="space-y-4"><p className="text-sm text-[var(--v2-ink-2)]">{t("minutes.errorBody", { defaultValue: "The read service is temporarily unavailable. Your meeting data was not changed." })}</p><V2Button size="sm" onClick={() => index.refetch()}><RefreshCw className="size-3.5" aria-hidden />{t("minutes.retry", { defaultValue: "Try again" })}</V2Button></V2PanelBody></V2Panel> : null}
      {!authError && index.data && meetings.length === 0 && hasOlder ? <V2Panel><V2PanelHead title={t("minutes.partialArchiveTitle", { defaultValue: "More meeting records are available" })} meta={t("minutes.partialArchiveMeta", { defaultValue: "Partial archive" })} /><V2PanelBody className="space-y-4"><p className="text-sm text-[var(--v2-ink-2)]">{t("minutes.partialArchiveBody", { defaultValue: "This page contains related Minutes items. Load the next page to continue to the meeting list." })}</p>{currentLoadOlder && loadOlder.isError ? <p className="text-xs text-[var(--v2-danger)]">{t("minutes.loadOlderError", { defaultValue: "Older meetings could not be loaded. The meetings above are still available." })}</p> : null}<V2Button size="sm" disabled={currentLoadOlder && loadOlder.isPending} onClick={() => nextCursor && loadOlder.mutate({ cursor: nextCursor, sessionRevision })}>{currentLoadOlder && loadOlder.isPending ? t("minutes.loadingOlder", { defaultValue: "Loading older meetings…" }) : t("minutes.loadOlder", { defaultValue: "Load older meetings" })}</V2Button></V2PanelBody></V2Panel> : null}
      {!authError && index.data && meetings.length === 0 && !hasOlder ? <V2Panel><V2PanelHead title={t("minutes.emptyTitle", { defaultValue: "No captured meetings yet" })} meta={t("minutes.emptyMeta", { defaultValue: "0 meetings" })} /><V2PanelBody><p className="text-sm text-[var(--v2-ink-2)]">{t("minutes.emptyBody", { defaultValue: "When a consented meeting is captured, its transcript and summary will appear here with their retention dates." })}</p></V2PanelBody></V2Panel> : null}
      ...
      {!authError && meetings.length > 0 ? <section className="grid min-h-[520px] gap-px bg-[var(--v2-hairline)] lg:grid-cols-[minmax(290px,0.78fr)_1.4fr]">
        ...
        })}</ul>{currentLoadOlder && loadOlder.isError ? <p className="mt-3 text-xs text-[var(--v2-danger)]">{t("minutes.loadOlderError", { defaultValue: "Older meetings could not be loaded. The meetings above are still available." })}</p> : null}{hasOlder ? <V2Button className="mt-3 w-full" size="sm" disabled={currentLoadOlder && loadOlder.isPending} onClick={() => nextCursor && loadOlder.mutate({ cursor: nextCursor, sessionRevision })}>{currentLoadOlder && loadOlder.isPending ? t("minutes.loadingOlder", { defaultValue: "Loading older meetings…" }) : t("minutes.loadOlder", { defaultValue: "Load older meetings" })}</V2Button> : null}</div>
        <div className="bg-[var(--v2-bg)] p-5 md:p-7">{currentDetail && detail.isPending ? <div className="h-56 animate-pulse border border-[var(--v2-hairline)] bg-[var(--v2-bg-raised)]" aria-label={t("minutes.loadingDetail", { defaultValue: "Loading meeting detail" })} /> : currentDetail && detail.isError ? renderDetailError() : currentDetail && detail.data ? <article><div className="mb-6 flex flex-wrap items-start justify-between gap-3 border-b border-[var(--v2-hairline)] pb-4"><div><div className="mb-2 flex items-center gap-2 font-mono text-[10px] uppercase text-[var(--v2-ink-3)]"><FileText className="size-3.5" aria-hidden />{kindLabel(detailKind(detail.data.item), t)}</div><h2 className="text-xl font-semibold">{detail.data.item.title}</h2></div><V2Badge><CalendarDays className="size-3" aria-hidden />{t("minutes.retainedUntil", { defaultValue: "Retained until" })} {dateLabel(detail.data.item.retention.expires_at, locale)}</V2Badge></div>{"capture_notice" in detail.data.item ? <div className="mb-6 flex flex-wrap items-center justify-between gap-2 border border-[var(--v2-hairline)] bg-[var(--v2-bg-raised)] px-3 py-2"><span className="flex items-center gap-2 text-xs font-medium"><ShieldCheck className="size-4 text-[var(--v2-accent)]" aria-hidden />{t("minutes.captureVerified", { defaultValue: "Visible capture verified" })}</span><span className="font-mono text-[10px] text-[var(--v2-ink-2)]">{detail.data.item.capture_notice.policy_version}</span></div> : null}{detailBody(detail.data.item, t, locale)}</article> : <div className="grid h-full min-h-72 place-items-center text-center"><div><FileText className="mx-auto mb-3 size-7 text-[var(--v2-ink-4)]" aria-hidden /><p className="text-sm text-[var(--v2-ink-3)]">{t("minutes.selectPrompt", { defaultValue: "Choose a summary or transcript to review it here." })}</p></div></div>}</div>
      </section> : null}
    </div>
  </main>;
}
