...
>;
const fetchTelosMock = fetchTelos as jest.MockedFunction<typeof fetchTelos>;
...
describe("Settings activation sections", () => {
  ...
  it("labels dream, mine, and user-scheduled jobs with next-run visibility", async () => {
      ...
      data: {
        jobs: [
          { id: "dream_3am", command: "dream", next_run_secs: 1_800_000_000 },
          {
            ...
            enabled: false,
            paused: true,
            next_run_secs: 1_800_001_800,
          },
          { id: "weekday-brief", name: "Weekday brief", next_run_secs: 1_800_003_600 },
          {
            id: "dream-journal-reminder",
            command: "remind me to write down my dream",
            name: "Dream journal reminder",
            next_run_secs: 1_800_007_200,
          },
        ],
      },
    });
    ...
    expect(screen.getByText("Dream journal reminder")).toBeInTheDocument();
    expect(screen.getAllByText("Dream reflection")).toHaveLength(1);
    expect(screen.getAllByText("Paused")).toHaveLength(2);
    expect(screen.getAllByText(/^Next run /)).toHaveLength(3);
  });
  ...
  it("renders the canonical allowlisted model picker and clears to the operator default", () => {
    ...
    expect(screen.getByRole("option", { name: /Claude Haiku 4\.5 · 200K · cost A/ })).toBeInTheDocument();
    fireEvent.change(picker, { target: { value: "" } });
    expect(onChange).toHaveBeenCalledWith(null);
  });

  it("renders stored allowlist aliases as their canonical model option", () => {
    render(<SettingsAgentModelPicker value="claude-opus-4-7" onChange={jest.fn()} />);
    expect(screen.getByRole("combobox", { name: "Default model" })).toHaveValue(
      "claude-opus-4.7",
    );
  });

  it("adopts only through the transition API, then removes the reviewed row", async () => {
  ...
  it("dismisses only through the transition API", async () => {
    fetchSuggestionsMock.mockResolvedValueOnce([
      { key: "durable_fact/behavior/2", origin: "trace-miner", content: "Keep replies compact" },
    ]);
    transitionSuggestionMock.mockResolvedValueOnce(undefined);
    ...
    await waitFor(() =>
      expect(transitionSuggestionMock).toHaveBeenCalledWith(
        "dismiss",
        "durable_fact/behavior/2",
      ),
    );
  });

  it("shows whether TELOS actively steers prompts", async () => {
    fetchTelosMock.mockResolvedValueOnce({
      items: [{ key: "durable_fact/telos/goal/1", type: "goal", content: "Launch ZAKI" }],
      telosInPrompt: true,
    });

    renderWithQueryClient(<SettingsTelosSection />);

    expect(await screen.findByText("Active in prompts")).toBeInTheDocument();
    expect(screen.getByText("Launch ZAKI")).toBeInTheDocument();
  });
